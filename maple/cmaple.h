...
public:
     ...
     */
    CMaple(std::string aln_filename, std::string format = "", std::string seqtype = "");
     ...
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int setModel(std::string model_name);
    ...
    /*! \brief Read a tree file
     *
     * Read an input tree from a file in NEWICK format
     * @param[in] tree_filename Name of a tree file
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int readTree(std::string tree_filename);
     ...
     * @param[in] output_filename Name of the output MAPLE file (optional)
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int extractMaple(std::string aln_filename, std::string output_filename = "");
     ...
     * @param[in] output_filename Name of the output FASTA file (optional)
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int extractFASTA(std::string aln_filename, std::string output_filename = "");
     ...
     */
    std::string getTreeString(std::string tree_type = "BIN");
     ...
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int setRef(std::string ref_filename);
    
    /*! \brief Set the tree search type
     *
     * Set the tree search type:
     * - "NONE"/"FAST": no tree search (placement only).
     * - "PARTIAL"/"NORMAL": only consider pruning branches at newly-added nodes when seeking SPR moves.
     * - "COMPLETE"/"SLOW": consider all nodes when seeking SPR moves.
     * @param[in] tree_search_type a type of tree search
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int setTreeSearchType(std::string tree_search_type);
    
    /*! \brief Enable short-range (shallow) tree search
     *
     * Enable short-range (shallow) tree search before applying a deeper tree search
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int enableShortRangeTreeSearch();
     ...
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int setMinBlength(double min_blength);
     ...
     * @return a status code: 0 - success, non-zero - failed with errors
     */
    int setThreshProb(double thresh_prob);
