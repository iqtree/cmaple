...
template void cmaple::PhyloNode::computeTotalLhAtNode<2>(std::unique_ptr<SeqRegions>&, PhyloNode&, const AlignmentBase*, const ModelBase*, const RealNumType, const bool, const RealNumType);
template void cmaple::PhyloNode::computeTotalLhAtNode<4>(std::unique_ptr<SeqRegions>&, PhyloNode&, const AlignmentBase*, const ModelBase*, const RealNumType, const bool, const RealNumType);
template void cmaple::PhyloNode::computeTotalLhAtNode<20>(std::unique_ptr<SeqRegions>&, PhyloNode&, const AlignmentBase*, const ModelBase*, const RealNumType, const bool, const RealNumType);
...
void cmaple::PhyloNode::computeTotalLhAtNode(std::unique_ptr<SeqRegions>& total_lh, PhyloNode& neighbor, const AlignmentBase* aln, const ModelBase* model, const RealNumType threshold_prob, const bool is_root, const RealNumType blength)
{
    // if node is root
    if (is_root)
...
const std::string cmaple::PhyloNode::exportString(const bool binary, const std::vector<std::string>& seq_names, const bool show_branch_supports)
{
    if (!isInternal())
    {
        ...
        else
        {
            ...
            if (binary)
            {
                ...
                output += "," + seq_names[less_info_seqs[num_less_info_seqs - 1]] + ":0" + closing_brackets;
            }
