...
class Alignment {
 public:
  ...
  enum InputType {
    ...
    IN_MAPLE,   /*!< [MAPLE](https://www.nature.com/articles/s41588-023-01368-0)
                   format */
    IN_AUTO,    /*!< Auto detect */
  ...
  /*! \brief Constructor from an alignment file in FASTA, PHYLIP, or
   * [MAPLE](https://www.nature.com/articles/s41588-023-01368-0) format
  ...
  /*! \brief Read an alignment from a stream in FASTA, PHYLIP, or
   * [MAPLE](https://www.nature.com/articles/s41588-023-01368-0) format
   * @param[in] aln_stream A stream of an alignment file
   * @param[in] ref_seq A reference sequence (optional). If not specified, it
   * will be read from the alignment (in MAPLE format) or automatically
  ...
  /*! \brief Read an alignment from a file in FASTA, PHYLIP, or
   * [MAPLE](https://www.nature.com/articles/s41588-023-01368-0) format
   * @param[in] aln_filename Name of an alignment file
   * @param[in] ref_seq A reference sequence (optional). If not specified, it
   * will be read from the alignment (in MAPLE format) or automatically
  ...
  /** \brief Write the alignment to a stream in FASTA, PHYLIP, or
   * [MAPLE](https://www.nature.com/articles/s41588-023-01368-0) format
  ...
  /** \brief Write the alignment to a file in FASTA, PHYLIP, or
   * [MAPLE](https://www.nature.com/articles/s41588-023-01368-0) format
   ...
   *
   * @throw std::logic\_error if the alignment is empty (i.e., nothing to write)
   * @throw std::ios\_base::failure if aln\_filename already exists and
   * overwrite = false
   */
  void write(const std::string& aln_filename,
  ...
  /**
   ...
   @return a raw state
   */
  static char convertState2Char(const cmaple::StateType& state,
 ...
 private:
   ...
   */
  cmaple::PositionType computeSeqDistance(Sequence& sequence,
   ...
   */
  void extractMutations(const cmaple::StrVector& sequences,
                        const cmaple::StrVector& seq_names,
   ...
   */
  void readFastaOrPhylip(std::istream& aln_stream,
                         const std::string& ref_seq = "");
   ...
   */
  void readFasta(std::istream& aln_stream,
                 cmaple::StrVector& sequences,
                 cmaple::StrVector& seq_names,
  ...
  /**
   ...
   @throw std::logic\_error if the alignment is in an incorrect format
   */
  void readPhylip(std::istream& aln_stream,
                  cmaple::StrVector& sequences,
                  cmaple::StrVector& seq_names,
                  bool check_min_seqs = true);
  ...
  /**
   ...
   @throw std::logic\_error if the alignment is in an incorrect format
   */
  void readSequences(std::istream& aln_stream,
                     cmaple::StrVector& sequences,
                     cmaple::StrVector& seq_names,
                     InputType aln_format = IN_AUTO,
                     bool check_min_seqs = true);
   ...
   */
  void processSeq(std::string& sequence,
                  std::string& line,
                  cmaple::PositionType line_num);

  /**
   Add a mutation to a sequence
   @throw std::logic\_error if sequence contains invalid states
   */
  void addMutation(Sequence* sequence,
                   char state_char,
                   cmaple::PositionType pos,
                   cmaple::PositionType length = -1);
