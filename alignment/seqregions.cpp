...
template double cmaple::SeqRegions::computeAbsoluteLhAtRoot<2>(
    const ModelBase*,
    const std::vector<std::vector<PositionType>>&);
template double cmaple::SeqRegions::computeAbsoluteLhAtRoot<4>(
    const ModelBase*,
    const std::vector<std::vector<PositionType>>&);
template double cmaple::SeqRegions::computeAbsoluteLhAtRoot<20>(
    const ModelBase*,
    const std::vector<std::vector<PositionType>>&);

template double cmaple::SeqRegions::computeSiteLhAtRoot<2>(
    std::vector<RealNumType>&,
    const ModelBase*,
    const std::vector<std::vector<PositionType>>&);
template double cmaple::SeqRegions::computeSiteLhAtRoot<4>(
    std::vector<RealNumType>&,
    const ModelBase*,
    const std::vector<std::vector<PositionType>>&);
template double cmaple::SeqRegions::computeSiteLhAtRoot<20>(
    std::vector<RealNumType>&,
    const ModelBase*,
    const std::vector<std::vector<PositionType>>&);

template double cmaple::SeqRegions::mergeTwoLowers<2>(
    ...
    const Alignment*,
    const ModelBase*,
    const RealNumType* const,
    const RealNumType,
    const bool) const;
template double cmaple::SeqRegions::mergeTwoLowers<4>(
    std::unique_ptr<SeqRegions>&,
    const RealNumType,
    const SeqRegions&,
    const RealNumType,
    ...
    const RealNumType,
    const bool) const;
template double cmaple::SeqRegions::mergeTwoLowers<20>(
    std::unique_ptr<SeqRegions>&,
    const RealNumType,
    const SeqRegions&,
    const RealNumType,
...
template void cmaple::SeqRegions::mergeUpperLower<2>(
    std::unique_ptr<SeqRegions>&,
    RealNumType,
    const SeqRegions&,
    RealNumType,
    ...
    const ModelBase*,
    const RealNumType) const;
template void cmaple::SeqRegions::mergeUpperLower<4>(
    std::unique_ptr<SeqRegions>&,
    RealNumType,
    const SeqRegions&,
    RealNumType,
    ...
    const ModelBase*,
    const RealNumType) const;
template void cmaple::SeqRegions::mergeUpperLower<20>(
    std::unique_ptr<SeqRegions>&,
    RealNumType,
    const SeqRegions&,
    RealNumType,
...
template void cmaple::SeqRegions::computeTotalLhAtRoot<2>(
    std::unique_ptr<SeqRegions>&,
    const ModelBase*,
    RealNumType) const;
template void cmaple::SeqRegions::computeTotalLhAtRoot<4>(
    std::unique_ptr<SeqRegions>&,
    const ModelBase*,
    RealNumType) const;
template void cmaple::SeqRegions::computeTotalLhAtRoot<20>(
    std::unique_ptr<SeqRegions>&,
    const ModelBase*,
    RealNumType) const;

template double cmaple::SeqRegions::calculateSiteLhContributions<2>(
    std::vector<RealNumType>&,
    std::unique_ptr<SeqRegions>&,
    const RealNumType,
    const SeqRegions&,
    ...
    const RealNumType* const,
    const RealNumType) const;
template double cmaple::SeqRegions::calculateSiteLhContributions<4>(
    std::vector<RealNumType>&,
    std::unique_ptr<SeqRegions>&,
    const RealNumType,
    const SeqRegions&,
    ...
    const RealNumType* const,
    const RealNumType) const;
template double cmaple::SeqRegions::calculateSiteLhContributions<20>(
    std::vector<RealNumType>&,
    std::unique_ptr<SeqRegions>&,
    const RealNumType,
    const SeqRegions&,
...
template <const StateType num_states>
void cmaple::merge_N_O(const RealNumType lower_plength,
...
template <const StateType num_states>
void cmaple::merge_O_N(const SeqRegion& reg_o,
  ...
  if (total_blength > 0) {
    auto new_lh =
        std::make_unique<SeqRegion::LHType>();  // = new
...
template <const StateType num_states>
void cmaple::merge_O_ORACGT(const SeqRegion& seq1_region,
                            const SeqRegion& seq2_region,
                            const RealNumType total_blength_1,
                            const RealNumType total_blength_2,
                            const PositionType end_pos,
  ...
  if (total_blength_1 > 0) {
    updateLHwithMat<num_states>(model->transposed_mut_mat,
  ...
  // seq1 = "O" and seq2 = R or ACGT
    ...
    if (total_blength_2 > 0) {
      ...
      sum_new_lh += updateVecWithState<num_states>(
          new_lh_value.data(), seq2_state, transposed_mut_mat_row,
          total_blength_2);
    } else {
...
template <const StateType num_states>
void cmaple::merge_RACGT_O(const SeqRegion& seq2_region,
                           const RealNumType total_blength_2,
                           const PositionType end_pos,
...
template <const StateType num_states>
void cmaple::merge_RACGT_RACGT(const SeqRegion& seq2_region,
                               const RealNumType total_blength_2,
                               const PositionType end_pos,
                               SeqRegion::LHType& new_lh,
                               const ModelBase* model,
...
template <const StateType num_states>
void cmaple::merge_RACGT_ORACGT(const SeqRegion& seq1_region,
                                const SeqRegion& seq2_region,
                                const RealNumType total_blength_1,
                                const RealNumType total_blength_2,
                                const RealNumType upper_plength,
  ...
  if (seq1_region.plength_observation2root >= 0) {
    ...
    updateLHwithMat<num_states>(model->transposed_mut_mat, root_vec, *new_lh,
                                length_to_root);
  } else {
    if (total_blength_1 > 0) {
...
template <const StateType num_states>
void cmaple::SeqRegions::mergeUpperLower(
    std::unique_ptr<SeqRegions>& merged_regions,
...
template <const StateType num_states>
auto cmaple::merge_O_O_TwoLowers(const SeqRegion& seq2_region,
                                 RealNumType total_blength_2,
                                 const PositionType end_pos,
                                 const Alignment* aln,
                                 const ModelBase* model,
...
template <const StateType num_states>
auto cmaple::merge_O_RACGT_TwoLowers(
    const SeqRegion& seq2_region,
    RealNumType total_blength_2,
    const PositionType end_pos,
    const Alignment* aln,
...
template <const StateType num_states>
auto cmaple::merge_O_ORACGT_TwoLowers(
    const SeqRegion& seq1_region,
    const SeqRegion& seq2_region,
    RealNumType total_blength_1,
    RealNumType total_blength_2,
  ...
  if (total_blength_1 > 0) {
    updateLHwithMat<num_states>(model->mutation_mat, *(seq1_region.likelihood),
...
template <const StateType num_states>
auto cmaple::merge_RACGT_O_TwoLowers(
    const SeqRegion& seq2_region,
    RealNumType total_blength_2,
    const PositionType end_pos,
    const Alignment* aln,
...
template <const StateType num_states>
auto cmaple::merge_RACGT_RACGT_TwoLowers(
    const SeqRegion& seq2_region,
    RealNumType total_blength_2,
    const PositionType end_pos,
    const Alignment* aln,
...
template <const StateType num_states>
auto cmaple::merge_RACGT_ORACGT_TwoLowers(
    const SeqRegion& seq1_region,
    const SeqRegion& seq2_region,
    RealNumType total_blength_1,
    RealNumType total_blength_2,
  ...
  if (total_blength_1 > 0) {
    ...
    setVecWithState<num_states>(new_lh->data(), seq1_state,
                                transposed_mut_mat_row, total_blength_1);
  } else {
...
template <const StateType num_states>
auto cmaple::merge_notN_notN_TwoLowers(
    const SeqRegion& seq1_region,
    const SeqRegion& seq2_region,
...
template <const StateType num_states>
RealNumType cmaple::SeqRegions::mergeTwoLowers(
    std::unique_ptr<SeqRegions>& merged_regions,
    const RealNumType plength1,
...
template <const StateType num_states>
RealNumType cmaple::SeqRegions::calculateSiteLhContributions(
...
template <const StateType num_states>
auto cmaple::SeqRegions::computeAbsoluteLhAtRoot(
    const ModelBase* model,
  ...
  for (const SeqRegion& region : regions) {
    ...
    if (region.type == TYPE_R) {
      for (StateType i = 0; i < num_states; ++i) {
        log_lh += model->root_log_freqs[i] *
                  (cumulative_base[region.position + 1][i] -
                   cumulative_base[start_pos][i]);
      }
    }
    // type ACGT
    ...
    // type O
      ...
      RealNumType tot = 0;
      tot +=
          dotProduct<num_states>(&(*region.likelihood)[0], model->root_freqs);
      log_factor *= tot;
    }
...
template <const StateType num_states>
RealNumType cmaple::SeqRegions::computeSiteLhAtRoot(
    std::vector<RealNumType>& site_lh_contributions,
    const ModelBase* model,
    const std::vector<std::vector<PositionType>>& cumulative_base) {
  // dummy variables
      ...
      for (PositionType pos = start_pos; pos < region.position + 1; ++pos) {
        for (StateType i = 0; i < num_states; ++i) {
          site_lh_contributions[pos] +=
              ...
              (cumulative_base[pos + 1][i] - cumulative_base[pos][i]);
        }
      }
    }
    // type ACGT
...
template <const StateType num_states>
void cmaple::SeqRegions::computeTotalLhAtRoot(
