...
class SeqRegion : public Mutation {
 ...
 private:
  ...
  /**
   *  Convert Ambiguious state into typical states: nucleotides/amino-acids...;
   * N; O; R
   *  @throw std::invalid\_argument if seq\_type is unknown/unsupported
   *  @throw std::logic\_error if invalid type of seqregion found
   */
  void convertAmbiguiousState(cmaple::SeqRegion::SeqType seq_type,
                              int max_num_states);

  /**
   *  Convert Ambiguious state (of DNA data) into typical states: A/C/G/T; N; O;
   * R
   *  @throw std::logic\_error if invalid type of seqregion found
   */
  ...
  /**
   *  Convert Ambiguious state (of AA data) into typical states: amino-acid; N;
   * O; R
   *  @throw std::logic\_error if invalid type of seqregion found
   */
 ...
 public:
  ...
  /**
   ...
   entry is (type = 1, pos = 234, plength_observation = 0.0001,
   plength_from_root = 0.0002) then this means that a "C" was observed at genome
   position 234, and the observation is separated from the root by a distance of
   0.0001, while a distance of 0.0002 separates the root from the current node
   (or position along a branch) considered.
   */
